...
ACTOR_MODEL_FILE = "./models/dueling_dqn_cartpole.pth"
...
class DuelingQNetwork(nn.Module):
    ...
    def forward(self, observation: torch.Tensor):
        ...
        value: torch.Tensor = self.value_stream(y)
        advantage: torch.Tensor = self.advantage_stream(y)
...
action_spec_size = 2
q_net = DuelingQNetwork(action_spec_size, device=device).train()
...
target_q_net = DuelingQNetwork(action_spec_size, device=device).eval()
...
def optimize_q_net(q_net: DuelingQNetwork, target_q_net: DuelingQNetwork, sample: TensorDict, update_target_net: bool, gamma=0.99, tau=0.01, l1_beta=1.0):
    with torch.no_grad():
...
for episode in range(2000):
    ...
    if is_training:
        ...
        plt.tight_layout()
        plt.savefig("./.tmp/dueling_dqn_cartpole.png")
        plt.close()
